...
//		http.csrf().ignoringAntMatchers("/user/**"); // 회원 관리 처리 API (POST /user/**) 에 대해 CSRF 무시
		...
		// image 폴더를 login 없이 허용
				/*
				 * .antMatchers("/images/**").permitAll() // css 폴더를 login 없이 허용
				 * .antMatchers("/css/**").permitAll() // 회원 관리 처리 API 전부를 login 없이 허용
				 * .antMatchers("/login/user/**").permitAll()
				 * .antMatchers("/club/**").permitAll() .antMatchers("/chatroom/**").permitAll()
				 * .antMatchers("/basic/**").permitAll()
				 * .antMatchers("/reservation/**").permitAll()
				 * .antMatchers("/place/**").permitAll() .antMatchers("/notice/**").permitAll()
